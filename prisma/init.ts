...
const NUM_WORKERS = os.cpus().length; // Tận dụng hết số nhân CPU
const RECORDS_PER_WORKER = Math.floor(TOTAL_RECORDS / NUM_WORKERS);

async function main() {
  console.log(`🚀 Bắt đầu seed 100M dòng với ${NUM_WORKERS} workers...`);
  ...
  for (let i = 0; i < NUM_WORKERS; i++) {
    const worker = new Worker(path.join(__dirname, 'worker.js'), {
      workerData: {
        workerId: i,
        recordsToCreate: RECORDS_PER_WORKER,
      },
    });
  ...
  console.log('✅ Hoàn thành 100 triệu dòng.');
}
